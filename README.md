# Electe Blog

Official blog repository for [Electe](https://electe.net) - AI-powered data analytics platform for European SMEs.

## 📖 About

This repository contains the markdown source files for our blog articles, automatically synced from Hashnode every hour.

**Read our blog**: [electe.net/news](https://www.electe.net/news)

Articles are also available on [Hashnode](https://fabiolauria.hashnode.dev) and [daily.dev](https://app.daily.dev/fabiolauria).

## 🎯 Topics

- AI insights and reflections
- Machine Learning & Data Analytics
- Business Intelligence for SMEs
- European Tech Ecosystem
- Innovation and entrepreneurship

## 📬 Newsletter

Join 100,000+ subscribers and get AI insights delivered to your inbox.

**[Subscribe here](https://magic.beehiiv.com/v1/648654e3-349f-4522-a74d-4bc3773c8864?email=)**

## 🔄 Automatic Sync

This repository uses GitHub Actions to automatically sync articles from Hashnode every hour. Articles are organized by year and month in the `articles/` directory.

Latest sync: Check the [Actions tab](../../actions) for the last successful run.

## 📝 Contributing

Found a typo or want to suggest improvements? Feel free to open an issue or submit a pull request.

## 🔗 Connect

- **Website**: [electe.net](https://electe.net)
- **LinkedIn**: [Electe](https://www.linkedin.com/company/electe/)
- **X/Twitter**: [@ElecteSrl](https://x.com/ElecteSrl)

---

© 2025 ELECTE S.R.L. - Built with ❤️ 
